...
const Lesson = () => {
  ...
  if (isLoading) {
    return <div className="p-8 text-center">Generating your AI lesson...</div>;
  }
  ...
  if (!lesson) {
    return (
      <div className="p-8 text-center">
        <CactusAvatar mood="sad" message="Topic not found!" size="lg" />
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full">
                Lesson {(topic.completedLessons?.length || 0)} of {topic.lessonsCount || topic.lessons || 5}
              </span>
            </div>
            <h1 className="text-2xl font-bold text-foreground mb-1">
              {content?.title || `Introduction to ${topic.title}`}
            </h1>
            <p className="text-sm text-muted-foreground">{topic.description}</p>
          </div>
          <CactusAvatar size="md" mood="thinking" message="Let's dive in!" />
        </div>
      </motion.div>
      ...
      <div className="space-y-6 mb-8">
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-lg font-semibold text-foreground">AI Generated Lesson</h2>
          </div>
