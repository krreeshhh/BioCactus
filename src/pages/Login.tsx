...
const Login = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const isNew = await loginWithEmail(email, password);
            toast.success("Welcome back!");
            // Even in login, check if they somehow missed onboarding, but usually existing users go to dashboard
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            // If isNew is null or false, it might mean redirect was triggered
            // or we're awaiting the Auth state change.
            if (isNew !== false) {
            ...
            <Card className="w-full max-w-md m3-glass border-white/5 relative z-10 p-2">
                <CardHeader className="space-y-2 text-center pb-8">
                    ...
                    <CardDescription className="text-muted-foreground font-medium">
                        Welcome back to the frontier of biotech
                    </CardDescription>
                </CardHeader>
